...
wss.on('connection', function connection(ws) {
    ...
    var myVar = setInterval(sendPosition, 200);
    function sendPosition() {
        ws.send(JSON.stringify({
            type: 'EGG_DATA',
            data: {
                egg_positionX: Math.random() * 100,
                egg_positionY: Math.random() * 300
            }
        }));
    }
    var myVar1 = setInterval(sendDirection, 500);
    function sendDirection() {
        ws.send(JSON.stringify({
            type: 'PLAYER_DATA_AI',
            data: {
                //generate a random direction
                playerAI_direction: Math.floor((Math.random() * 4)),
